#!/usr/bin/env bash
# Preflight pré-tarefa: estado do repo, branch, arquivos modificados, secrets
# aparentes e resumo do projeto. Read-only. Nunca edita arquivos.
set -euo pipefail
...
hits="$(grep -rInE "$secret_re" \
        ...
        --exclude-dir=node_modules --exclude-dir=.idea --exclude-dir=.claude \
        --exclude='*.example' \
        "$REPO_ROOT" 2>/dev/null | head -20 || true)"
if [ -n "$hits" ]; then
  warn "Possíveis secrets encontrados — REVISE antes de continuar:"
  # Mascara o lado do valor; mostra só arquivo:linha e a chave.
