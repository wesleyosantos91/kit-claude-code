#!/usr/bin/env bash
# Roda PIT mutation testing e cobra score >= 90% quando configurado.
# Avisa (sem falhar) quando o PIT não está configurado ou quando ainda não há
# código de negócio para mutar ("No mutations found").
set -euo pipefail
...
case "$BUILD_TOOL" in
  maven|gradle)
    if [ "$BUILD_TOOL" = "maven" ] && ! maven_has 'pitest'; then
      warn "PIT ausente no pom.xml (meta ${MIN}%). Regras: $QUALITY_CONFIG_DIR/pitest/pitest-rules.md"; exit 0
    fi
    if [ "$BUILD_TOOL" = "gradle" ] && ! gradle_has 'pitest'; then
      warn "Plugin pitest ausente no Gradle. Regras: $QUALITY_CONFIG_DIR/pitest/pitest-rules.md"; exit 0
    fi
